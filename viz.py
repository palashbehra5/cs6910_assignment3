...
def plot_attention_heatmap_truncated(model, test_dataset):
    ...
    for i in range(9):
        ...
        ax = axs[i // 3, i % 3]
        xticks = np.arange(0,len(list(target[i])),1)
        yticks = np.arange(0,len(list(source[i])),1)
        xticklabels = list(target[i])
        yticklabels = list(source[i])
        ax.set_xticks(xticks)
        ax.set_yticks(yticks)
        ax.set_xticklabels(xticklabels, fontproperties = font_prop_hindi)
        ax.set_yticklabels(yticklabels, fontproperties = font_prop_english)
        ax.imshow((attention_weights[i].cpu().detach().numpy())[1:len(list(source[i]))+1,1:len(list(target[i]))+1])
