...
def plot_confusion_matrix(y_pred, y_true):
    ...
    font_prop = FontProperties(fname='fonts\\NotoSerifDevanagari-Medium.ttf', size=12)
...
def plot_attention_heatmap(model, test_dataset, targets):

    font_prop = FontProperties(fname='fonts\\NotoSerifDevanagari-Medium.ttf')
    ...
    for i in range(9):
        ...
        ax = axs[i // 3, i % 3]
        hmap = (np.log(attention_weights[i].cpu().detach().numpy())+1)
        ax.imshow(hmap,cmap = 'Blues')
...
def plot_attention_heatmap_truncated(model, test_dataset):

    source = [i.split(',')[0] for i in open('aksharantar_sampled\hin\hin_test.csv', encoding='utf-8')]
    target = [i.split(',')[1][:-1] for i in open('aksharantar_sampled\hin\hin_test.csv', encoding='utf-8')]

    font_prop_hindi = FontProperties(fname='fonts\\NotoSerifDevanagari-Medium.ttf')
    font_prop_english = FontProperties(fname='fonts\\Roboto-Medium.ttf')
    ...
    for i in range(9):
        ...
        ax = axs[i // 3, i % 3]
        xticks = np.arange(0,len(list(source[i])),1)
        yticks = np.arange(0,len(list(target[i])),1)
        xticklabels = list(source[i])
        yticklabels = list(target[i])
        ax.set_xticks(xticks)
        ax.set_yticks(yticks)
        ax.set_xticklabels(xticklabels, fontproperties = font_prop_english)
        ax.set_yticklabels(yticklabels, fontproperties = font_prop_hindi)
        hmap = (np.log(attention_weights[i].cpu().detach().numpy())+1)
        #hmap = attention_weights[i+50].cpu().detach().numpy()
        ax.imshow(hmap[1:len(list(target[i]))+1,1:len(list(source[i]))+1],cmap = 'Blues')

    #plt.subplots_adjust(top=4.5, hspace=4)
    #plt.suptitle('Attention Heatmaps for the First 10 Data Points in the Hindi-Test Set, Generated by the Trained Model with the Best Configuration')
    plt.tight_layout()
    plt.show()